...
//补洞
extern void mesh_hole_fill(TriMesh *themesh);
//检查网格模型
extern int check_mesh(TriMesh *in);
// Color by distance to another mesh
extern void dist2mesh(TriMesh *mesh, TriMesh *othermesh, float maxdist);
extern void dist2mesh(TriMesh *mesh, TriMesh *othermesh, float maxdist,vector<float> &dists);
// Color based on normals
extern void colorbynormals(TriMesh *mesh);
// Find closest point to p on face i of mesh
extern point closest_on_face(const TriMesh *mesh, int i, const point &p);
//Calculate mesh volume
extern float mesh_volume(TriMesh* mesh);
...
extern int			FindInterSection(vec2 P0, vec2 D0, vec2 P1, vec2 D1, vec2 I[2],float &pu);							//两线段求交
extern bool			write_stl(TriMesh *mesh, FILE *f);
