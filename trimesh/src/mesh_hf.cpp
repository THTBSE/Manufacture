...
// Find the neighbor field in a FaceStruct that points to i, and change  it to point to j
...
vector<hole> *find_holes(edgeset *edges)
{
	...
	while (!edges->empty()) 
	{
		...
		while (1) 
		{
			...
			if (hi != newhole.end()) 
			{
				// Assuming everything is OK topologically,  this could only have been caused if there
				// was a choice of ways to go the last time,  we encountered this vertex.  Obviously,
				// we chose the wrong way, so find a different  way to go.
...
void print_holes(const vector<hole> *holes, bool verbose)
{
	...
	sort(holesizes.begin(), holesizes.end(), std::greater<int>());
	...
	vector<FaceStruct> &tris)
{
	...
	for (int iter = 0; iter < 2*n; iter++) 
	{
		...
		if (tris[i].n12 != NO_FACE) {
			...
			if (should_flip(themesh, meanedgelen, newverts,	v1, v0, v2, v3)) 
			{
				if (match1 == 1 &&   edges.find(make_pair(tris[i].v3, tris[j].v2)) == edges.end()) 
				{
					...
					tris[i].n23 = j;
					tris[j].n12 = i;
				} else if (match1 == 2 &&	   edges.find(make_pair(tris[i].v3, tris[j].v3)) == edges.end()) 
				{
					edges.erase(make_pair(tris[i].v1, tris[i].v2));
					edges.erase(make_pair(tris[i].v2, tris[i].v1));
					...
					tris[i].n23 = j;
					tris[j].n23 = i;
				} else if (match1 == 3 &&	   edges.find(make_pair(tris[i].v3, tris[j].v1)) == edges.end()) 
				{
					edges.erase(make_pair(tris[i].v1, tris[i].v2));
					edges.erase(make_pair(tris[i].v2, tris[i].v1));
		...
		if (tris[i].n23 != NO_FACE) 
		{
			...
			if (should_flip(themesh, meanedgelen, newverts,		v2, v0, v3, v1)) 
			{
				if (match2 == 1 &&    edges.find(make_pair(tris[i].v1, tris[j].v2)) == edges.end()) 
				{
					...
					tris[i].n31 = j;
					tris[j].n12 = i;
				} else if (match2 == 2 &&	   edges.find(make_pair(tris[i].v1, tris[j].v3)) == edges.end()) 
				{
					edges.erase(make_pair(tris[i].v2, tris[i].v3));
					edges.erase(make_pair(tris[i].v3, tris[i].v2));
					...
					tris[i].n31 = j;
					tris[j].n23 = i;
				} else if (match2 == 3 &&	   edges.find(make_pair(tris[i].v1, tris[j].v1)) == edges.end()) 
				{
		...
		if (tris[i].n31 != NO_FACE) 
		{
			...
			if (should_flip(themesh, meanedgelen, newverts,		v3, v0, v1, v2)) 
			{
				if (match3 == 1 &&	    edges.find(make_pair(tris[i].v2, tris[j].v2)) == edges.end()) 
				{
					...
					tris[i].n12 = j;
					tris[j].n12 = i;
				} else if (match3 == 2 &&	   edges.find(make_pair(tris[i].v2, tris[j].v3)) == edges.end()) 
				{
					edges.erase(make_pair(tris[i].v3, tris[i].v1));
					edges.erase(make_pair(tris[i].v1, tris[i].v3));
					...
					tris[i].n12 = j;
					tris[j].n23 = i;
				} else if (match3 == 3 &&	   edges.find(make_pair(tris[i].v2, tris[j].v1)) == edges.end()) 
				{
					edges.erase(make_pair(tris[i].v3, tris[i].v1));
					edges.erase(make_pair(tris[i].v1, tris[i].v3));
...
//
void mesh_hole_fill(TriMesh *themesh)
{
	bool fillonly = false;
	bool listonly = false;
	bool nocurv = false;
	...
	float mel = 2.0 * themesh->feature_size();
	...
	for (unsigned int i = 0; i < holes->size(); i++)
	{
		...
		fill_hole(themesh, mel, newverts,	initial_edge_neighbors,	(*holes)[i], tmptris);
		if (!fillonly) 
		{
			...
			relax_triangulation(themesh,
				...
				copy(tmptris.begin(), tmptris.end(), std::back_inserter(newtris));
	}
	...
	{
		themesh->faces.push_back(TriMesh::Face(newtris[i].v1, newtris[i].v2, newtris[i].v3));
	}
