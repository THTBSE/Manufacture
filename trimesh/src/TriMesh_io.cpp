...
TriMesh* TriMesh::read_stl_helper(const char* filename)
{
...
//---------OVERWRITE CODE FOR REMOVE DUPLICATED VETICES------------------------
//---------IT'S FASTER THAN ORIGINAL CODE----------------------------------------
//---------WRITTEN BY Heap_Frank IN 2014---------------------------------------
class vtx_comp
{
public:
...
TriMesh* TriMesh::load_stl2(const char *filename)
{
	TriMesh *mesh = new TriMesh();
	...
	for (size_t i=0; i<trif.size(); i++)
	{
		for (int j=0; j<3; j++)
		{
			itrmdp = rmdp.find(vtx[trif[i][j]]);
			if (itrmdp == rmdp.end())
			{
				rmdp.insert(make_pair(vtx[trif[i][j]],kk));
...
//---------------------------------------------------------------
TriMesh *TriMesh::read_stl(const char* filename)
{
	typedef std::set<fyVec3f>		    fyVec3fSet;
	typedef std::vector<fyVec3f>		fyVec3fVector;
	typedef std::map<fyVec3f,long>		fyVec3fLongMap;
	...
	fyLongVector   iv;
	fyVec3fLongMap vm;    //vm是一个map,把顶点作为键值就可以自动去除冗余。iv用来存每个顶点的索引
	                      //iv的个数仍然是未去除冗余的顶点个数
	long nPP = vexp.size();
	iv.resize(nPP);
	for(long ip=0; ip<nPP; ip++)
	{
		...
		{
			long last_index = vm.size(); //
			vm.insert(fyVec3fLongMap::value_type(*pV, last_index));
			iv[ip] = last_index;
		}
	}
				 ...
				 vector<TriMesh::Face> &tris)
{
	...
	for (unsigned i = 2; i < thisface.size(); i++)
		tris.push_back(TriMesh::Face(thisface[0],
		thisface[i-1],
		thisface[i]));
}
...
bool TriMesh::write_stl(const char* fname)
{
	...
	fprintf(fp,"solid FCL_STL\n");

	for (unsigned i=0; i<faces.size(); i++)
	{
		fprintf(fp," facet normal %f %f %f\n",FaceNormal[i][0],FaceNormal[i][1],FaceNormal[i][2]);
