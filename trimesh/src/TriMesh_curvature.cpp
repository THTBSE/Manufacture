...
double	CalcArea(point  A, point B, point C)
{
	...
	vec ac=B-C;
	vec crs=ab CROSS ac;
	double area=double(0.5*len(crs));
	return area;
}
//
void Calcuvw(point P,point A, point B, point C, double &u, double &v, double &w)
{
	double s=CalcArea(A,B,C);
	double spbc=CalcArea(P,B,C);
	double spca=CalcArea(P,C,A);
	double spab=CalcArea(P,A,B);

	u=spbc/s;
	v=spca/s;
	w=spab/s;
}
//计算两个区间[u0, u1]和[v0, v1]相交。如果不相交，则函数的返回值为0；如果只有一个交点，返回1；
//相交于一个区间，则返回值为2，区间的端点保存在w[0]和w[1]中
...
//两线段求交算法——计算机图形学几何工具算法详解P173
//如果不相交，返回0；只有一个交点，返回1；两线段重叠返回2
int FindInterSection(vec2 P0, vec2 D0, vec2 P1, vec2 D1, vec2 I[2],float &pu)
{
	vec2 E=P1-P0;
	float kross=D0[0]*D1[1]-D0[1]*D1[0];
	float sqrkross=kross*kross;
	float sqrLen0=len2(D0);
	float sqrLen1=len2(D1);
	float sqrEpsilon = 1e-6;

	if (sqrkross>sqrEpsilon*sqrLen0*sqrLen1)
	{
		float s=(E[0]*D1[1]-E[1]*D1[0])/kross;
		if (s<0 || s>1)
		{
			return 0;
		}
		float t=(E[0]*D0[1]-E[1]*D0[0])/kross;
		if (t<0 || t>1)
		{
			return 0;
		}
