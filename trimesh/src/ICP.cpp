		      ...
		      float &maxdist, int verbose)
{
	...
	if (verbose > 1) {
		printf("Computed overlaps in %.2f msec.\n",
			(now() - t) * 1000.0f);
	}
}
		   ...
		   int verbose, bool do_affine)
{
	...
	if (verbose > 1) 
	{
		printf("sqrt covariance 1 =");
		for (int j = 0; j < 3; j++) {
			printf("\n\t");
			for (int k = 0; k < 3; k++)
				printf("%10.3f ", csqrt1[j][k]);
		}
		printf("\nsqrt covariance 2 =");
		for (int j = 0; j < 3; j++) 
		{
		      ...
		      bool do_scale, bool do_affine)
{
	...
	if (verbose > 1) 
	{
		printf("Generated %lu pairs in %.2f msec.\n",
	...
	if (verbose > 1)
	{
		printf("RMS point-to-plane error = %f\n", err);
		for (int i = 0; i < 5; i++)
			if (eval[i] < EIG_THRESH * eval[5])
				printf("Small eigenvalue %f (largest is %f)\n", eval[i], eval[5]);
	}
	...
	if (verbose > 1) 
	{
		printf("Computed xform in %.2f msec.\n",
			(t4-t3) * 1000.0f);
	}
	...
	if (verbose > 1) 
	{
		printf("Updated CDFs in %.2f msec.\n",
			(t5-t4) * 1000.0f);
	}
		      ...
		      float &incr, bool trans_only)
{
	...
	if (verbose > 1)
		printf("RMS point-to-point error = %f\n", err);
	  ...
	  bool do_scale /* = false */, bool do_affine /* = false */)
{
	...
	{
		timestamp tnow = now();
		printf("Time for initial iterations: %.2f msec.\n\n",
		       (tnow-t) * 1000.0f);
		t = tnow;
	}
	if (err < 0.0f)
	...
	do {
		...
		if (verbose > 1) {
			timestamp tnow = now();
			printf("Time for this iteration: %.2f msec.\n\n",
			       (tnow-t) * 1000.0f);
			t = tnow;
		}
		if (err < 0.0f)
